...
\UzLThesisSetup{
  ...
  Abstract              = {
   The confidentiality of sensitive data requires special protection, especialy in the medical area, where dealing with patients data. For this purpose, access control mechanisms are used to deny requests to sensitive data. However, these mechanisms do not take into account that by combining statistcal information and results of smart chosen queries on non-sensitve data, information on sensitive data can still be inferred. One solution to prevent an attacker from inferring sensitive data is the Database Inference Control (DBIC). A DBIC mechanism is the prototype Angerona, which uses definitions of probabilistic dependency rules to deny access to sensitive data. In this work, Angerona is evaluated, since it has not yet been empirically evaluated on real data. This will be done by  testing the security and runtime on real medical data. It was found, that the runtime for real world use cases takes to long to start (offline) and to answer queries (online) to be used in practice.
  },
  Zusammenfassung       = {
  	Die Vertraulichkeit sensibler Daten erfordert einen besonderen Schutz, insbesondere im medizinischen Bereich, in dem mit Patientendaten gearbeitet wird. Dafür werden Zugangskontrollmechanismen verwendet, um Anfragen an sensible Daten zu verweigern. Bei diesen Mechanismen wird jedoch nicht beachtet, dass durch Kombinierung von statistischen Auswertungen und Ergebnisse klug gewählter Anfragen auf unsensible Daten, trotzdem Informationen zu sensiblen Daten hergeleitet werden können. Eine Lösung, um zu verhindern, dass ein Angreifer auf sensible Daten schließen kann, ist die Database Inference Control (DBIC). Ein DBIC-Mechanismus ist der Prototyp Angerona, der anhand von Definitionen von probabilistischen Abhängigkeitsregeln, den Zugriff auf sensible Daten verweigert. In dieser Arbeit wird Angerona evaluiert, da dieser bisher  noch nicht auf reellen Daten empirisch evaluiert wurde. Dafür wird, mit reellen medizinischen Daten, die Sicherheit getestet und die Laufzeit gemessen. Dabei wurde festgestellt, dass Angerona zu lange Laufzeiten hat, um zu starten (Offline) und Anfragen zu beantworten (Online), um in der Praxis verwendet zu werden.
  },
...
\begin{algorithm}[ht]
	\caption{Angerona}\label{alg-angerona}
	\begin{algorithmic}[1]
...
\begin{figure}[ht]
	\caption{Bayes-Netz mit zwei Abhängigkeiten}
